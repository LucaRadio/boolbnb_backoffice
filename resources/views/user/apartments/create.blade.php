        ...
        <form action="{{route ('user.apartments.store')}}" class="form-group w-75 d-inline-block shadow rounded-3 p-3 py-5"
         method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="number" step="1" min="0" class="form-control text-center w-75 mx-auto" name="n_rooms">

            </div>
            <div class="mb-3">
                <label class="form-label">Numero di bagni</label>
                <input type="number" step="1" min="0" class="form-control text-center w-75 mx-auto" name="n_bathrooms">

            </div>
            <div class="mb-3">
                <label class="form-label">Numero di letti</label>
                <input type="number" step="1" min="0" class="form-control text-center w-75 mx-auto" name="n_beds">

            </div>
            <div class="mb-3">
                <label class="form-label">Metri quadrati</label>
                <input type="number" step="0.5" min="30" class="form-control text-center w-75 mx-auto" name="square_meters">
                ...
                @foreach ($services as $service)
                  ...
                  @error('services') is-invalid @enderror">
                    <input class="form-check-input @error('services') is-invalid @enderror" type="checkbox"
                      ...
                      >
                    <label class="form-check-label" for="serviceCheckbox_{{ $loop->index }}">{{ $service->name }}</label>
                  </div>
                @endforeach
{{--     
                @error('technologys')
